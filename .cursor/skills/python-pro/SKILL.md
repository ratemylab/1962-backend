...
| Logging & Config | `references/logging-config.md` | Structured logging, correlation ID, config from env |
...
- Configuration and secrets from environment, validated at startup; single settings entry point (see `references/logging-config.md`)
- Structured logging with request/correlation ID where applicable for request-scoped flows (see `references/logging-config.md`)
